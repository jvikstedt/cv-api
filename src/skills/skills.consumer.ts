...
import { QUEUE_NAME_SKILLS, QUEUE_NAME_CV, EventType, CONFIG_QUEUE, CONFIG_QUEUE_CV_RELOAD } from '../constants';

const queueConfig = config.get(CONFIG_QUEUE);
const cvReloadDelay = queueConfig[CONFIG_QUEUE_CV_RELOAD];

@Processor(QUEUE_NAME_SKILLS)
export class SkillsConsumer {
  private readonly logger = new Logger(SkillsConsumer.name);
